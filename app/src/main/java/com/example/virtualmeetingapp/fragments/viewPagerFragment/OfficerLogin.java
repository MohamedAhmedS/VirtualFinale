                                ...
                                if (task.isSuccessful()) {
                                    ...
                                    db.collection(Constants.COLLECTION_USER).document(str_email).get().addOnCompleteListener(fetchUserTask -> {
                                        if (fetchUserTask.isSuccessful() && fetchUserTask.getResult() != null) {
                                            Officer currentUser = fetchUserTask.getResult().toObject(Officer.class);

                                            if (currentUser != null && currentUser.getUserType().equals(Constants.USER_TYPE_OFFICER)) {

//                                                Global.updateDeviceToken(currentUser.getUid());
                                                new SystemPrefs().saveUserSession(currentUser, Constants.USER_TYPE_OFFICER);
                                                Intent intent = new Intent(getContext(), OfficersActivity.class);
                                                startActivity(intent);
                                                getActivity().finish();
                                            } else {
                                                Toast.makeText(getActivity(), "Only Officers can login.", Toast.LENGTH_SHORT).show();
                                            }
                                        } else {
...
//                                    db.collection(Constants.USERS_COLLECTION).whereEqualTo("userType", userType).get().addOnCompleteListener(checkType -> {
//                                        if (!checkType.isSuccessful()) {
//                                            Toast.makeText(getContext(), checkType.getException().getMessage(),
//                                                    Toast.LENGTH_SHORT).show();
//                                        } else {
//                                            Intent intent = new Intent(getContext(), OfficersActivity.class);
//                                            startActivity(intent);
